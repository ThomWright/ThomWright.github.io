...
In a distributed system, when service A sends a request to service B to do a task, there are three possible outcomes:

1. **Success** – service A is informed that the task was done.
2. **Failure** – service A is informed that the task was *not* done.
3. **Uncertainty** – the response does not explicitly indicate either success or failure, or service A never receives a response (or does not receive a response in a reasonable time frame).

Often the third case is forgotten about. Services respond with either a successful response or an error, and an error is assumed to mean that the task was not done. That assumption isn't safe.

It's important to be clear on what guarantees service A can rely on for each outcome, and for service B to communicate these correctly.
...
What should service A do when the state of a task is uncertain? Here are a few ideas:
