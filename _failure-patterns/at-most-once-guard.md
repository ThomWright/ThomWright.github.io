...
The existence of the record means “the operation was attempted”, but does not give any information about success or failure. It can be helpful to combine with a [recovery point]({% link _failure-patterns/recovery-point.md %}) or a [response record]({% link _failure-patterns/response-record.md %}) to signify "the operation was completed, and the result was success/failure". Since the system might crash before writing one of these records, their absence does not necessarily mean the operation failed or did not occur. If a retry sees the guard record (pre-operation) but no recovery point or response record (post-operation), it should respond with appropriate uncertainty.
